...
from .uni_tool import Dictionary, logger, pad_1d_tokens, pad_2d, pad_coords, WEIGHT_DIR, weight_download, TransformerEncoderWithPair, MODEL_CONFIG
    ...
    def batch_collate_fn(self, samples):
        ...
        for k in samples[0].keys():
            if k == 'src_coord':
                ...
                v = pad_2d(
                    [torch.tensor(s[k]).long() for s in samples],
            ...
            batch[k] = v
        try:
            label = torch.tensor([s for s in labels])
        except:
            label = None

        data = {'unimol_input': batch, 'target': label}
        return MoleculeDataset(data)

    def batch_collate_fn_2(self, samples):
        """
        Collate function for GGT-style training with variable-length Uni-Mol inputs.
        Pads src_tokens, src_coord, src_distance, src_edge_type to the max atom
        count in the batch before stacking.
        """

        # ============================================================
        # 1. PyG graph batch
        # ============================================================
        graph = Batch.from_data_list(samples)

        # ============================================================
        # 2. UniMol input batch — with padding for variable-length molecules
        # ============================================================
        unimol_input_list = [d.unimol_input for d in samples]

        # Convert all to tensors first
        src_tokens_list = [torch.as_tensor(x["src_tokens"], dtype=torch.long) for x in unimol_input_list]
        src_distance_list = [torch.as_tensor(x["src_distance"], dtype=torch.float32) for x in unimol_input_list]
        src_coord_list = [torch.as_tensor(x["src_coord"], dtype=torch.float32) for x in unimol_input_list]
        src_edge_type_list = [torch.as_tensor(x["src_edge_type"], dtype=torch.long) for x in unimol_input_list]

        # Find the maximum sequence length (N+2) in this batch
        max_len = max(t.size(0) for t in src_tokens_list)

        def pad_1d(tensor, max_len, pad_value=0):
            """Pad a 1D tensor [L] -> [max_len]"""
            pad_size = max_len - tensor.size(0)
            return F.pad(tensor, (0, pad_size), value=pad_value)

        def pad_2d_square(tensor, max_len, pad_value=0):
            """Pad a 2D square tensor [L, L] -> [max_len, max_len]"""
            L = tensor.size(0)
            pad_size = max_len - L
            # Pad last dim first, then second-to-last dim
            return F.pad(tensor, (0, pad_size, 0, pad_size), value=pad_value)

        def pad_2d_coord(tensor, max_len, pad_value=0):
            """Pad a 2D coord tensor [L, 3] -> [max_len, 3]"""
            pad_size = max_len - tensor.size(0)
            return F.pad(tensor, (0, 0, 0, pad_size), value=pad_value)

        unimol_input = {
            "src_tokens": torch.stack(
                [pad_1d(t, max_len, pad_value=1) for t in src_tokens_list],  # 1 = PAD token index
                dim=0,
            ),
            "src_distance": torch.stack(
                [pad_2d_square(t, max_len, pad_value=0.0) for t in src_distance_list],
                dim=0,
            ),
            "src_coord": torch.stack(
                [pad_2d_coord(t, max_len, pad_value=0.0) for t in src_coord_list],
                dim=0,
            ),
            "src_edge_type": torch.stack(
                [pad_2d_square(t, max_len, pad_value=0) for t in src_edge_type_list],
                dim=0,
            ),
        }
        ...
        label_list = []
        for d in samples:
            if torch.is_tensor(d.y):
                y = d.y.float().view(-1)
            else:
                y = torch.tensor(d.y, dtype=torch.float32).view(-1)
            label_list.append(y)
...
from .uni_tool import DataHub2, YamlHandler
...
if __name__ == '__main__':
    ...
    model = UniMolModel(
        output_dim=2, data_type="molecule", remove_hs=False
    ).to(device)

    DATA_CSV_path = 'bace1.csv'
    config_path = os.path.join(os.path.dirname(__file__), 'config/default.yaml')
    yamlhandler = YamlHandler(config_path)
    config = yamlhandler.read_yaml()

    datahub = DataHub2(
        data=DATA_CSV_path, is_train=True, save_path='exp', **config
    )
    data = datahub.data

    molecular_dataset = MoleculeDataset(data)

    train_loader = DataLoader(molecular_dataset,
                              batch_size=8,
